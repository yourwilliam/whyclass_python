...
First, read about the dice game Pig \[here]\([https://en.wikipedia.org/wiki/Pig\_(dice\_game](https://en.wikipedia.org/wiki/Pig_\(dice_game))). Then, write the function playPig(), that allows two players to play the game Pig. You will want to use random.randint(1,6) to randomly choose a number between 1 and 6 inclusive. Grading criteria:
