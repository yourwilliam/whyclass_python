...
`filter(**kwargs)` 根据kwargs过滤的内容返回一个QuerySet。

`exclude(**kwargs)` 根据kwargs排除过滤的内容返回一个QuerySet。
...
经常我们在使用查询语句的时候，希望看一下最终的sql的查询语句是什么。 比较好的方式是在debug的断点中停住查看，这样更方便调试 ![-w1261](http://ossp.pengjunjie.com/mweb/15591895495754.jpg)
